...
function bk_widgets_backend_enqueue()
{
    ...
    $translation = array(
        'btn_text' => esc_html__( 'Processing...', 'automotive-store' ),
...
class Autoparts_Notice_Handler {
	...
	public function bk_getting_started() {
        ...
        if ( ! current_user_can( 'install_plugins' ) ) {
            $status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'automotive-store' );
        ...
        if( $request == 2 && strpos( $slug, 'automotive-store' ) === false ){
            wp_send_json_error( );
        }
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'automotive-store' );
