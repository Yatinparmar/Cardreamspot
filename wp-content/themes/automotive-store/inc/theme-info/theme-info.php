...
if ( ! class_exists( 'Autoparts_Theme_Info' ) ) {
    class Autoparts_Theme_Info{
        ...
        public function prepare_class() {
            ...
            $this->page_title    = $this->theme_name . esc_html__( ' Info', 'automotive-store' );
            $this->menu_title    = $this->theme_name . esc_html__( ' Theme', 'automotive-store' );
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'automotive-store' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'automotive-store' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'automotive-store' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'automotive-store' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions', 'automotive-store').'</a>';
                }
                echo '</div>';
            }
        }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download', 'automotive-store' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview', 'automotive-store' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<thead>';
                    echo '<tr>';
                    echo '<th>' . esc_html__( 'Theme Features','automotive-store' ) . '</th>';
                    echo '<th>' . esc_html__( 'Free','automotive-store' ) . '</th>';
                    echo '<th>' . esc_html__( 'Pro (Premium)','automotive-store' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
...
$config = array(
    ...
    // Main welcome title
    'info_title' => sprintf( esc_html__( 'Welcome to %s - ', 'automotive-store' ), $theme_name_config ),

    // Main welcome content
    'info_content' => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help and you enjoy using it!', 'automotive-store' ), '<b>'.$theme_name_config.'</b>' ),
    ...
    'quick_links' => array(
        'theme_url'  => array(
            'text' => __( 'Theme Info', 'automotive-store' ),
            'url' => esc_url( 'https://www.templatehouse.net/product/auto-parts-wordpress-theme/' )
        ),
        'pro_url'  => array(
            'text' => __( 'Buy Pro', 'automotive-store' ),
            'url' => esc_url( 'https://www.templatehouse.net/product/auto-parts-wordpress-theme/' )
        ),
    ),

    'tabs' => array(
        'getting_started'      => esc_html__( 'Getting Started', 'automotive-store' ),
        'free_pro'             => esc_html__( 'Compare Free Vs Pro', 'automotive-store' ),
        'recommended_plugins'  => esc_html__( 'Recommended Plugins', 'automotive-store' ),
    ...
    'gs_steps' => array(
        ...
        'two' => array(
            'title' => esc_html__( 'Pre-built Demos', 'automotive-store' ),
            'desc' => esc_html__( 'We recommend utilizing our collection of pre-designed starter site templates as an efficient and convenient way to begin your project.', 'automotive-store' ),
            'button_label' => esc_html__( 'Browse Demos', 'automotive-store' ),
        ...
        'seven' => array(
            'title' => esc_html__( 'Need WordPress Theme Developemnt or Customization?', 'automotive-store' ),
            'desc' => esc_html__( 'We Create Beautiful & SEO Friendly WordPress Websites. Theme Customization to Custom WordPress Plugin Development.', 'automotive-store' ),
            'button_label' => esc_html__( 'Contact Now', 'automotive-store' ),
        ...
        'five' => array(
            'title' => esc_html__( 'Love Our Product?', 'automotive-store' ),
            'desc' => esc_html__( 'We would greatly appreciate your motivation through a 5-star rating on our profile.', 'automotive-store' ),
            'button_label' => esc_html__( 'Rate Us', 'automotive-store' ),
            'button_link' => esc_url( 'https://wordpress.org/support/theme/' .$theme_name_config_url. '/reviews' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'six' => array(
            'title' => esc_html__( 'Have Question? Ask Us, Now!', 'automotive-store' ),
            'desc' => esc_html__( 'We are committed to resolving any issues that may arise to ensure a seamless experience while using our theme.', 'automotive-store' ),
            'button_label' => esc_html__( 'Get Support', 'automotive-store' ),
    ...
    'free_pro' => array(
        array(
            'desc'=> __( 'Access to All Starter Sites', 'automotive-store' ),
            'free' => __( 'no','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ), 
        array(
            'desc'=> __( 'Access to All Templates & Patterns', 'automotive-store' ),
            'free' => __( 'no','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Work with the Page Builders', 'automotive-store' ),
            'free' => __( 'no','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Creative & Unique Blocks', 'automotive-store' ),
            'free' => __( 'no','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Elementor Ready', 'automotive-store' ),
            'free' => __( 'no','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Priority Updates', 'automotive-store' ),
            'free' => __( 'no','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Priority Support', 'automotive-store' ),
            'free' => __( 'no','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Full Site Editing', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Customize Everything', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Lighting Fast', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'One-click Demo Installer', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Fully Responsive', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Clean & Optimized Code', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Multiple Site Styles', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Global Settings', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'CSS Animation', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Gutenberg Ready', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'eCommerce Ready', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'SEO Optimized', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'Multilingual Compatibility', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
        array(
            'desc'=> __( 'RTL/LTR Language Support', 'automotive-store' ),
            'free' => __( 'yes','automotive-store' ),
            'pro'  => __( 'yes','automotive-store' ),
        ),
    ),
